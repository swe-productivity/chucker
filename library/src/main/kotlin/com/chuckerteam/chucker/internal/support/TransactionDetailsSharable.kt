...
internal class TransactionDetailsSharable(
    ...
    override fun toSharableContent(context: Context): Source =
        Buffer().apply {
            writeUtf8(
                "${context.getString(R.string.chucker_url)}: ${
            ...
            writeUtf8(
                "${context.getString(
                    R.string.chucker_request_time,
                )}: ${transaction.requestDateString}\n",
            )
            writeUtf8(
                "${context.getString(
                    R.string.chucker_response_time,
            ...
            writeUtf8(
                "${context.getString(
                    R.string.chucker_request_size,
                )}: ${transaction.requestSizeString}\n",
            )
            writeUtf8(
                "${context.getString(
                    R.string.chucker_response_size,
                )}: ${transaction.responseSizeString}\n",
            )
            writeUtf8(
                "${context.getString(
                    R.string.chucker_total_size,
